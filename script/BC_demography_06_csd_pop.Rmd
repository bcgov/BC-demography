...
df_bc_pop <- df_1710014201 %>% 
  ...
  mutate(csd = str_sub(dguid, start = 10, end = 13))
...
```{r}
df_bc_total <- df_bc_pop %>% 
  filter(geo == "British Columbia") %>% 
  # filter for relevant years (previous commissions)
  filter(ref_date %in% c("2008", "2015", "2020")) %>% 
  mutate(pop_change = value - lag(value, 2),
         pop_change_pct = round((pop_change / lag(value, 2) * 100), 2))
...
...CSD might be more useful


```{r}
df_bc_csd <- df_bc_pop2 %>% 
  group_by(csd, ref_date) %>% 
  summarise(csd_pop = sum(value)) %>% 
  ungroup() %>% 
  group_by(csd) %>% 
  # change since last commission
  mutate(pop_change = csd_pop - lag(csd_pop, 1),
         pop_change_pct = round((pop_change / lag(csd_pop, 1) * 100), 2)) %>% 
  ungroup()
